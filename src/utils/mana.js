...
const isDate = (string = "") => {
    ...
    if (!parts || parts.length !== 6) return false;
...
const findFirstChild = jsonOrArray => {
    if (_.isArray(jsonOrArray)) return { type: "array", item: jsonOrArray[0] };
...
const mergeChildren = json => {
    ...
    if (!isNumber(firstKey)) return identifyObject(json);
...
const findSimilarity = (json1, json2) => {
    return (
        ((100 * identicalFieldsCount(json1, json2)) /
            maxFieldsCount(json1, json2)) *
        k(json1, json2)
    );
};
...
const identicalFieldsCount = (json1, json2) => {
    return _.intersection(Object.keys(json1), Object.keys(json2)).length;
};

const maxFieldsCount = (json1, json2) => {
    return Math.max(Object.keys(json1).length, Object.keys(json2).length);
};
...
const mergeSimilarEntities = json => {
    ...
    entitiesKeys.forEach(key1 => {
        json[key1].__mergeWith = entitiesKeys.reduce((output2, key2) => {
            if (key1 !== key2) {
                const match =
                    ((100 * identicalFieldsCount(json[key1], json[key2])) /
                        maxFieldsCount(json[key1], json[key2])) *
                    k(json[key1], json[key2]);
